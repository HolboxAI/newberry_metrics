...
- Automatic live-updating Streamlit dashboard for visualizing metrics.
...
When you initialize `TokenEstimator`, it will automatically attempt to launch the Newberry Metrics dashboard in the background. The dashboard URL (typically `http://localhost:8501`) will be printed to your console.
...
# The dashboard should now be running. Check your console for the URL.
...
- **Automatic Launch**: The dashboard starts when `TokenEstimator` is initialized. The URL (default: `http://localhost:8501`) is printed to the console.
...
- **Shutdown**: The dashboard process is managed by `TokenEstimator` and should shut down automatically when your main Python script exits.
